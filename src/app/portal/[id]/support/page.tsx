import Link from "next/link";
import prisma from "@/shared/lib/db";

export const revalidate = 0;

export default async function SupportPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  
  let projectTitle = "Eastside Logistics Center";
  try {
    const proj = await prisma.project.findUnique({ where: { id } });
    if (proj) projectTitle = proj.title;
  } catch (e) {
    // Fallback Mock
  }

  return (
    <div className="bg-surface font-body-md text-on-surface min-h-screen flex flex-col selection:bg-primary selection:text-on-primary">
      
      {/* Top Header */}
      <header className="fixed top-0 w-full h-16 bg-surface/80 backdrop-blur-xl border-b border-[#334155] z-50 flex items-center justify-between px-6 md:px-8">
        <div className="flex items-center gap-4">
          <Link href="/" className="flex items-center gap-3">
            <img alt="AtlasBuild Logo" className="h-7 w-7 object-contain" src="/images/logo.png" />
            <span className="font-headline-md font-bold text-lg text-on-surface">AtlasBuild</span>
          </Link>
          <span className="text-[#334155]">|</span>
          <span className="font-label-sm text-xs text-on-surface-variant uppercase font-mono font-bold">
            {projectTitle}
          </span>
        </div>

        <div className="flex items-center gap-4">
          <Link 
            href={`/portal/${id}`} 
            className="px-4 py-1.5 border border-secondary text-secondary text-xs font-bold rounded-lg hover:bg-secondary hover:text-black transition-all flex items-center gap-1.5"
          >
            <span className="material-symbols-outlined text-[16px]">arrow_back</span>
            Back to Workspace
          </Link>
        </div>
      </header>

      {/* Main Content */}
      <main className="pt-24 pb-16 px-6 md:px-12 max-w-3xl mx-auto w-full flex-1 flex flex-col gap-8">
        
        <div>
          <h1 className="font-headline-lg text-2xl md:text-3xl text-on-surface font-bold">
            24/7 Site Engineering Support Channel
          </h1>
          <p className="font-body-md text-sm text-on-surface-variant mt-1">
            Direct high-priority communication with your assigned Lead Civil Structural Engineer.
          </p>
        </div>

        <div className="bg-surface-container/40 backdrop-blur-xl border border-white/5 rounded-xl p-6 md:p-8 flex flex-col gap-6">
          
          <div className="flex items-center gap-4 p-4 bg-primary/10 border border-primary/30 rounded-lg">
            <span className="material-symbols-outlined text-primary text-[28px]">support_agent</span>
            <div>
              <span className="font-headline-md text-sm text-on-surface font-bold">Assigned Lead Engineer: Marcus Brody, P.E.</span>
              <p className="text-xs text-on-surface-variant">Response SLA: Under 15 Minutes for Structural Inquiries</p>
            </div>
          </div>

          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-xs font-mono uppercase text-on-surface-variant">Ticket Priority</label>
              <select className="bg-surface-container-high border border-white/10 rounded-lg p-3 text-sm text-on-surface focus:outline-none focus:border-primary">
                <option>Normal Inquiry (Blueprint / Documentation)</option>
                <option>Urgent (Site Change Order / Field Condition)</option>
                <option>Critical (Safety / OSHA Compliance)</option>
              </select>
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-xs font-mono uppercase text-on-surface-variant">Support Request Details</label>
              <textarea 
                rows={4}
                placeholder="Describe your inquiry or requested structural change order..."
                className="bg-surface-container-high border border-white/10 rounded-lg p-3 text-sm text-on-surface focus:outline-none focus:border-primary placeholder:text-outline/50"
              />
            </div>

            <button 
              type="button"
              className="px-6 py-3 bg-primary text-black font-bold uppercase text-xs rounded-lg hover:bg-primary-container transition-all flex items-center justify-center gap-2 mt-2"
            >
              Submit Support Inquiry
              <span className="material-symbols-outlined text-[18px]">send</span>
            </button>
          </div>

        </div>

      </main>

    </div>
  );
}
