import Link from "next/link";
import prisma from "@/shared/lib/db";

export const revalidate = 0;

export default async function FeedbackPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  
  let projectTitle = "Eastside Logistics Center";
  try {
    const proj = await prisma.project.findUnique({ where: { id } });
    if (proj) projectTitle = proj.title;
  } catch (e) {
    // Fallback Mock
  }

  return (
    <div className="bg-surface font-body-md text-on-surface min-h-screen flex flex-col selection:bg-primary selection:text-on-primary">
      
      {/* Top Header */}
      <header className="fixed top-0 w-full h-16 bg-surface/80 backdrop-blur-xl border-b border-[#334155] z-50 flex items-center justify-between px-6 md:px-8">
        <div className="flex items-center gap-4">
          <Link href="/" className="flex items-center gap-3">
            <img alt="AtlasBuild Logo" className="h-7 w-7 object-contain" src="/images/logo.png" />
            <span className="font-headline-md font-bold text-lg text-on-surface">AtlasBuild</span>
          </Link>
          <span className="text-[#334155]">|</span>
          <span className="font-label-sm text-xs text-on-surface-variant uppercase font-mono font-bold">
            {projectTitle}
          </span>
        </div>

        <div className="flex items-center gap-4">
          <Link 
            href={`/portal/${id}`} 
            className="px-4 py-1.5 border border-secondary text-secondary text-xs font-bold rounded-lg hover:bg-secondary hover:text-black transition-all flex items-center gap-1.5"
          >
            <span className="material-symbols-outlined text-[16px]">arrow_back</span>
            Back to Workspace
          </Link>
        </div>
      </header>

      {/* Main Content */}
      <main className="pt-24 pb-16 px-6 md:px-12 max-w-3xl mx-auto w-full flex-1 flex flex-col gap-8">
        
        <div>
          <h1 className="font-headline-lg text-2xl md:text-3xl text-on-surface font-bold">
            Client Executive Feedback Portal
          </h1>
          <p className="font-body-md text-sm text-on-surface-variant mt-1">
            Share feedback on project milestones, engineering site transparency, and portal user experience.
          </p>
        </div>

        <div className="bg-surface-container/40 backdrop-blur-xl border border-white/5 rounded-xl p-6 md:p-8 flex flex-col gap-6">
          
          <div className="flex flex-col gap-2">
            <label className="text-xs font-mono uppercase text-on-surface-variant">Overall Milestone Rating</label>
            <div className="flex gap-3 text-secondary">
              {[1, 2, 3, 4, 5].map((star) => (
                <button key={star} type="button" className="p-2 bg-secondary/10 border border-secondary/20 rounded-lg hover:bg-secondary hover:text-black transition-all">
                  <span className="material-symbols-outlined text-[24px]">star</span>
                </button>
              ))}
            </div>
          </div>

          <div className="flex flex-col gap-2">
            <label className="text-xs font-mono uppercase text-on-surface-variant">Executive Comments / Remarks</label>
            <textarea 
              rows={4}
              placeholder="Provide comments regarding overall field execution and communication..."
              className="bg-surface-container-high border border-white/10 rounded-lg p-3 text-sm text-on-surface focus:outline-none focus:border-secondary placeholder:text-outline/50"
            />
          </div>

          <button 
            type="button"
            className="px-6 py-3 bg-secondary text-black font-bold uppercase text-xs rounded-lg hover:bg-secondary-container transition-all flex items-center justify-center gap-2 mt-2"
          >
            Submit Feedback
            <span className="material-symbols-outlined text-[18px]">rate_review</span>
          </button>

        </div>

      </main>

    </div>
  );
}
